...
class CometBFTMempoolClient:
    ...
    def CheckTx(self, tx_b64: str, cb: callable, tx_info: dict) -> None:
        ...
        try:
            ...
            if "result" in rpc_result:
                ...
                logger.error(
                    f"CometBFT RPC error for broadcast_tx_sync: Code={error_details.get('code')}, Message={error_details.get('message')}, Data={error_details.get('data')}")
                cb(MockResponseCheckTx(code=error_details.get('code', -1),
                                       log=f"RPC Error: {error_details.get('message')}"))
                with metrics_lock:
...
DISPATCHED_EVENTS_MAX = 50
recently_dispatched_events = deque(maxlen=DISPATCHED_EVENTS_MAX)

def get_event_signature(event_name: str, payload_b64: str) -> str:
    signature = f"{event_name}|{payload_b64}"
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if line.startswith("Name:"):
                    ...
                    if "payload_full" in current_event_info:
                        logger.warning("Payload already exists; resetting stale event info.")
                        current_event_info = {}
                    ...
                    try:
                        ...
                        if not payload_b64.strip():
                            logger.warning("Parsed payload is empty. Skipping.")
                            continue

                        current_event_info["payload_full"] = payload_b64
                        logger.debug(f"Payload base64 length: {len(payload_b64)}")

                    except Exception as e:
                        logger.error(f"Error converting hex payload to base64: {e}. Line: {line}")
                        current_event_info["payload_full"] = "Parsing Error"

                    event_name = current_event_info["name"]
                    payload_b64 = current_event_info["payload_full"]
                    event_signature = get_event_signature(event_name, payload_b64)
                    if event_signature in recently_dispatched_events:
                        logger.debug(f"Skipping duplicate event: {event_name}")
                        continue
                    recently_dispatched_events.append(event_signature)

                    logger.info(f"Parsed Serf user event: Name='{event_name}', Payload(base64)='{payload_b64[:30]}...'")
                    ...
                    try:
                        ...
                        logger.error(f"Failed to decode base64 payload for '{event_name}': {e}")
                        with metrics_lock:
                            activity_entry["cometbft_response"] = f"Decode Error: {e}"
                        continue

                    def check_tx_response_callback(response: MockResponseCheckTx, activity_entry=activity_entry, event_name_for_log=event_name):
                        with metrics_lock:
                            if response.code == 0:
                                logger.info(f"CometBFT RPC Broadcast Success: '{event_name_for_log}' Code={response.code}")
                                activity_entry["cometbft_response"] = f"Success (Code: {response.code}) Hash: {response.hash[:10]}..."
                            else:
                                logger.error(f"CometBFT RPC Broadcast Failed: '{event_name_for_log}' Code={response.code}")
                                activity_entry["cometbft_response"] = f"Failed (Code: {response.code}) Log: {response.log[:50]}..."

                    mempool_client.CheckTx(payload_b64, check_tx_response_callback, {})
                    current_event_info = {}
                    parsing_event_info_block = False
                    ...
                    try:
                        ...
                        if len(parts) != 2:
                            logger.debug(f"Could not parse event line: {line}")
                            continue
                        event_name, payload_b64 = parts[0], parts[1]
                        ...
                        logger.info(f"Parsed Serf user event (single line): Name='{event_name}', Payload(base64)='{payload_b64[:30]}...'")
                        with metrics_lock:
                            app_metrics["serf_events_received"] += 1
                            activity_entry = {
                                "timestamp": time.strftime('%Y-%m-%d %H:%M:%S'),
                        ...
                        try:
                            ...
                            logger.error(f"Failed to decode single-line payload for '{event_name}': {e}")
                            with metrics_lock:
                                activity_entry["cometbft_response"] = f"Decode Error: {e}"
                            continue
                        ...
                        logger.error(f"Error parsing single-line user event: {e}")
                        parsing_event_info_block = False
                        current_event_info = {}
                    ...
                    if "[INFO] agent:" in line or "[INFO] serf:" in line:
                        ...
                        logger.debug(f"Other Serf monitor line (ignored): {line}")
                    parsing_event_info_block = False
                    current_event_info = {}
