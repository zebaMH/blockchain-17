...
def serf_monitor_thread(serf_exec_path: str, rpc_addr: str, mempool_client: CometBFTMempoolClient):
    ...
    MEMBER_CHECK_INTERVAL = 10 # seconds (more frequent for responsiveness)
    COMETBFT_STATUS_CHECK_INTERVAL = 5 # seconds (more frequent for responsiveness)
    ...
    while True: # Keep the monitoring alive, restart if command exits
        ...
        if current_time - last_members_check_time > MEMBER_CHECK_INTERVAL:
            try:
                # Use serf members -format=json for structured output
                ...
                if members_process.returncode == 0:
                    ...
                    with metrics_lock:
                        ...
                        # Ensure our RPC status reflects actual success
    ...
    return render_template_string("""
        ...
        <head>
            ...
            <style>
                ...
                .text-indigo-600 { color: #667eea; } /* Adjust indigo for dark theme */
                .text-blue-600 { color: #63b3ed; } /* Adjust blue for dark theme */
                .text-purple-800 { color: #b794f4; } /* Adjust purple for dark theme */
                .text-green-800 { color: #68d391; } /* Adjust green for dark theme */
                .text-gray-900 { color: #f7fafc; } /* Very light text */
                .text-gray-700 { color: #cbd5e0; } /* Slightly darker light text */
                .text-gray-600 { color: #a0aec0; } /* Even darker light text */
                .text-gray-500 { color: #718096; } /* Default light gray */

                /* Status dots */
                .bg-green-500 { background-color: #48bb78; }
                .bg-red-500 { background-color: #f56565; }
                .bg-gray-400 { background-color: #a0aec0; }
                .bg-gray-700 { background-color: #4a5568; } /* For unknown status */

                /* Card backgrounds for different sections */
                .bg-blue-50 { background-color: #2a4365; border-color: #4299e1; } /* Dark blue for serf */
                .bg-purple-50 { background-color: #44337a; border-color: #9f7aea; } /* Dark purple for cometbft */
                .bg-green-50 { background-color: #2f855a; border-color: #68d391; } /* Dark green for members */
                .bg-gray-50 { background-color: #2d3748; border-color: #4a5568; } /* Dark gray for log */
        ...
        <body class="bg-gray-900 min-h-screen flex items-center justify-center p-4">
            <div class="card p-8 rounded-lg w-full max-w-4xl">
                <h1 class="text-4xl font-extrabold header-text mb-6 text-center">
                    ✨ Serf <span class="text-gray-200">↔</span> CometBFT Bridge Dashboard ✨
                </h1>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    <!-- Serf Monitoring Card -->
                    <div class="card bg-blue-50 p-6 flex flex-col">
                        <h2 class="text-xl font-semibold text-blue-200 mb-4">Serf Monitoring</h2>
                        <div class="flex items-center mb-2">
                            <span class="inline-block h-4 w-4 rounded-full {{ serf_status_color }} mr-2 animate-pulse"></span>
                            <span class="text-gray-200 font-medium">RPC Connection:</span>
                            <span class="ml-2 font-bold text-gray-50">{{ metrics.serf_rpc_status }}</span>
                        </div>
                        <p class="text-gray-300 mb-2">
                            Events Received: <span class="font-bold text-indigo-300">{{ metrics.serf_events_received }}</span>
                        </p>
                        <p class="text-gray-400 text-sm italic">
                            Monitor Thread: <span class="font-bold">{{ metrics.serf_monitor_status }}</span>
                        </p>
                        {% if metrics.serf_monitor_last_error %}
                        <p class="text-red-400 text-xs mt-1">Error: {{ metrics.serf_monitor_last_error }}</p>
                        {% endif %}
                    </div>

                    <!-- CometBFT Status Card -->
                    <div class="card bg-purple-50 p-6 flex flex-col">
                        <h2 class="text-xl font-semibold text-purple-200 mb-4">CometBFT Integration</h2>
                        <div class="flex items-center mb-2">
                            <span class="inline-block h-4 w-4 rounded-full {{ comet_status_color }} mr-2 animate-pulse"></span>
                            <span class="text-gray-200 font-medium">RPC Connection:</span>
                            <span class="ml-2 font-bold text-gray-50">{{ metrics.cometbft_rpc_status }}</span>
                        </div>
                        <p class="text-gray-300 mb-2">
                            Transactions Broadcast: <span class="font-bold text-indigo-300">{{ metrics.cometbft_tx_broadcast }}</span>
                        </p>
                        <p class="text-gray-400 text-sm italic">
                            RPC URL: <span class="font-bold text-blue-300">{{ cometbft_rpc_url }}</span>
                        </p>
                    ...
                    <div class="card bg-green-50 p-6 flex flex-col">
                        <h2 class="text-xl font-semibold text-green-200 mb-4">Serf Cluster Members ({{ metrics.serf_members|length }} Nodes)</h2>
                        {% if metrics.serf_members %}
                            <ul class="space-y-2 text-sm max-h-48 overflow-y-auto">
                                {% for member in metrics.serf_members %}
                                    <li class="flex items-center bg-gray-800 p-2 rounded-md shadow-inner">
                                        <span class="inline-block h-3 w-3 rounded-full {{ member.display_status_color }} mr-2"></span>
                                        <span class="font-medium text-gray-50">{{ member.name }}</span>
                                        <span class="text-gray-400 ml-2">({{ member.addr }}:{{ member.port }})</span>
                                        <span class="text-xs font-semibold ml-auto px-2 py-0.5 rounded-full {% if member.status == 'alive' %}bg-green-700 text-green-100{% elif member.status == 'failed' %}bg-red-700 text-red-100{% else %}bg-gray-700 text-gray-100{% endif %}">
                                            {{ member.status }}
                                        </span>
                                    </li>
                                {% endfor %}
                        ...
                        {% else %}
                            <p class="text-gray-500 italic text-center">No Serf members discovered yet. Ensure your topology is deployed and agents are joined!</p>
                        {% endif %}
                    </div>
                </div>
                ...
                <div class="card bg-gray-50 p-6 mb-8">
                    <h2 class="text-xl font-semibold text-gray-200 mb-4">Recent Activity Log</h2>
                    {% if activity_log %}
                        <div class="space-y-4 max-h-80 overflow-y-auto">
                            {% for entry in activity_log %}
                            <div class="bg-gray-800 border border-gray-700 rounded-md p-3 shadow-sm flex flex-col text-sm">
                                <div class="flex justify-between items-center mb-1">
                                    <p class="font-medium text-gray-50">{{ entry.timestamp }} - <span class="text-indigo-400">{{ entry.type }}</span>: <span class="font-bold">{{ entry.name }}</span></p>
                                    <span class="font-semibold {% if 'Success' in entry.cometbft_response %}text-green-400{% elif 'Failed' in entry.cometbft_response or 'Error' in entry.cometbft_response %}text-red-400{% else %}text-gray-400{% endif %} text-xs px-2 py-0.5 rounded-full">
                                        {{ entry.cometbft_response }}
                                    </span>
                                </div>
                                <p class="text-gray-300 text-xs payload-text">Payload: <span class="font-mono text-gray-400">{{ entry.payload_full }}</span></p>
                            </div>
                            {% endfor %}
                        </div>
                    {% else %}
                ...
                <div class="card bg-gray-800 p-4 rounded-md">
                    <p class="text-sm font-medium text-gray-200 mb-1">Application Configuration:</p>
                    <p class="text-xs break-all text-gray-300">Serf Executable Path: <span class="font-mono text-gray-100">{{ serf_exec_path }}</span></p>
                    <p class="text-xs break-all text-gray-300">Serf RPC Address: <span class="font-mono text-gray-100">{{ serf_rpc_addr }}</span></p>
                    <p class="text-xs break-all font-bold mt-2 text-gray-300">CometBFT RPC URL: <span class="font-mono text-blue-400">{{ cometbft_rpc_url }}</span></p>
                </div>
