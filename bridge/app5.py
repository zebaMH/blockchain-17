...
# This ensures logger is defined before any other part of the script tries to use it,
# and sets the default logging level to DEBUG for verbose output.
...
class CometBFTMempoolClient:
    ...
    def CheckTx(self, tx_bytes: bytes, cb: callable, tx_info: dict) -> None:
        ...
        try:
            logger.debug(f"Attempting to broadcast transaction (hash: {tx_b64[:10]}...) to CometBFT RPC: {endpoint}")
...
def serf_monitor_thread(serf_exec_path: str, rpc_addr: str, mempool_client: CometBFTMempoolClient):
    ...
    # State variables for multi-line parsing of Serf monitor output
    current_event_info = {} # Temporarily store Name and Payload
    ...
    while True: # Keep the monitoring alive, restart if command exits
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                # --- NEW: Multi-line parsing logic for Serf events ---
                # Based on observed 'Name:' and 'Payload:' lines appearing *before* or *around* 'Received event: user-event:'
                if line.startswith("Name:"):
                    parsing_event_info_block = True # Start capturing info for a new event
                    current_event_info = {"type": "Serf User Event"}
                    current_event_info["name"] = line.split("Name:", 1)[1].strip().strip('"')
                elif line.startswith("Payload:"):
                    if parsing_event_info_block: # Only process if we think we're in an event info block
                        try:
                            hex_bytes_str = line.split("Payload: []byte{", 1)[1].strip("}").replace("0x", "").replace(", ", "")
                            current_event_info["payload_full"] = base64.b64encode(bytes.fromhex(hex_bytes_str)).decode('utf-8')
                        except Exception as e:
                            logger.error(f"Error converting hex payload to base64: {e}. Line: {line}")
                            current_event_info["payload_full"] = "Parsing Error"

                        # Process the event here as soon as Payload is found,
                        # assuming Name was either found or will be handled if not.
                        if "name" in current_event_info and "payload_full" in current_event_info:
                            event_name_to_process = current_event_info["name"]
                            event_payload_b64_to_process = current_event_info["payload_full"]

                            logger.info(f"Parsed Serf user event: Name='{event_name_to_process}', Payload(base64)='{event_payload_b64_to_process[:30]}...'")

                            with metrics_lock:
                                app_metrics["serf_events_received"] += 1
                                new_activity = {
                                    "timestamp": time.strftime('%Y-%m-%d %H:%M:%S'),
                                    "type": current_event_info["type"],
                                    "name": event_name_to_process,
                                    "payload_full": event_payload_b64_to_process,
                                    "payload_preview": event_payload_b64_to_process[:50] + ("..." if len(event_payload_b64_to_process) > 50 else ""),
                            ...
                            try:
                                decoded_tx_bytes = base64.b64decode(event_payload_b64_to_process)
                            except Exception as e:
                                logger.error(f"Failed to base64 decode payload for event '{event_name_to_process}': {e}")
                                with metrics_lock:
                                    new_activity["cometbft_response"] = f"Decode Error: {e}"
                                parsing_event_info_block = False # Reset state
                                current_event_info = {}
                                continue

                            def check_tx_response_callback(response: MockResponseCheckTx, activity_entry=new_activity, event_name_for_log=event_name_to_process):
                                with metrics_lock:
                                    if response.code == 0:
                                        logger.info(
                            ...
                            # Reset parsing state after successful processing
                            parsing_event_info_block = False
                            current_event_info = {}

                        else: # Payload found, but Name not (or vice-versa), so not a complete event block
                            logger.debug(f"Incomplete event info block (missing Name or Payload): {line}")
                            # Keep parsing_event_info_block True, hoping for Name on next line (if Payload came first)
                elif "Received event: user-event:" in line:
                    # This is the "Received event: user-event: <name>" line if it's the only info
                    # We will try to parse it as a fallback if Name/Payload lines are not explicit.
                    # This block is moved AFTER Name/Payload checks to prioritize multi-line parsing
                    if not parsing_event_info_block: # Only process if we aren't already in a block
                        try:
                            # Attempt to split "user-event: <name> <payload>" format (legacy/simpler events)
                            event_part = line.split("Received event: user-event:", 1)[1].strip()
                            parts = event_part.split(" ", 1)
                            if len(parts) == 2: # Name and Payload found on one line
                                event_name_to_process = parts[0].strip()
                                event_payload_b64_to_process = parts[1].strip()

                                logger.info(f"Parsed Serf user event (single line): Name='{event_name_to_process}', Payload(base64)='{event_payload_b64_to_process[:30]}...'")
                                ...
                                tx_info = {}
                                mempool_client.CheckTx(decoded_tx_bytes, check_tx_response_callback, tx_info)
                                parsing_event_info_block = False # Ensure reset
                                current_event_info = {}
                            else:
                    ...
                    parsing_event_info_block = False # Reset parsing state if no recognized event info line
                    current_event_info = {}
