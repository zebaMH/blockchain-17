    ...
    while True:
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                # --- START Multi-line parsing logic for Serf events ---
                if "Received event: user-event:" in line:
                    parsing_event_info_block = True
                    current_event_info = {"type": "Serf User Event"} # Initialize for this event
                    # We expect Name and Payload to follow
                elif parsing_event_info_block and line.startswith("Name:"):
                    current_event_info["name"] = line.split("Name:", 1)[1].strip().strip('"')
                elif parsing_event_info_block and line.startswith("Payload:"):
                    try:
                        ...
                        current_event_info["payload_full"] = "Parsing Error" # Indicate issue
                    
                    # --- Process the event as soon as Payload is found (assuming Name is already parsed or not needed for direct dispatch) ---
                    if "name" in current_event_info and "payload_full" in current_event_info:
                        ...
                        try:
                            ...
                            # Reset parsing state after handling
                            parsing_event_info_block = False
                            current_event_info = {}
                            continue
                        ...
                        logger.debug(f"Event Info block complete but missing Name or Payload: {line}")
                        parsing_event_info_block = False # Reset state, as something went wrong
                        current_event_info = {}

                # If we were in a parsing block but this line is not Name/Payload or a new event, reset
                elif parsing_event_info_block and not (line.startswith("Name:") or line.startswith("Payload:") or "Received event: user-event:" in line or "Event Info:" in line or "Coalesce:" in line or "Event:" in line or "LTime:" in line):
                    logger.debug(f"Exiting event info parsing block for line: {line}")
                    parsing_event_info_block = False
                    current_event_info = {}
                else:
                    # General Serf monitor output (e.g., [INFO] agent.ipc: Accepted client)
                    if not parsing_event_info_block: # Only process if not in an event info block
